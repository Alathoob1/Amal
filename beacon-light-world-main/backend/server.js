...
const JWT_SECRET = 'aura_super_secret_key_123!';
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        const fullDetails = await db.getUserDetails(user.id);
        
        await db.addLog({ action: `تسجيل دخول: ${user.email}`, role: user.role, status: 'success' });
        res.json({ success: true, role: user.role, name: user.name, email: user.email, token, profile: fullDetails });
    } catch (err) {
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        const user = await db.createUser({ name, email: email.trim(), password, phone, childName, childAge, childGender, childLevel, childCom, childHistory, childTriggers, childNotes, childTherapy });
